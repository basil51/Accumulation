...
import type { AccumulationSignal } from '@/lib/types';

type DetailedAccumulationSignal = AccumulationSignal & {
  evidence?: any;
  eventIds?: string[];
};

export default function AccumulationSignalPage() {
  ...
  const [signal, setSignal] = useState<DetailedAccumulationSignal | null>(null);
  ...
  useEffect(() => {
    const loadSignal = async (id: string) => {
      ...
      setError(null);
      try {
        const data = await api.getAccumulationSignal(id);
        setSignal(data as DetailedAccumulationSignal);
      } catch (err: any) {
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-6">
          ...
          <p className="text-sm text-zinc-500 dark:text-zinc-400">
            Chain: {signal.coin.chain || 'Unknown'} • Created at:{' '}
        ...
        <div className="grid gap-6 md:grid-cols-2">
          ...
          <section className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg p-4">
            ...
            <dl className="space-y-3 text-sm">
              ...
              <div className="flex items-center justify-between">
                <dt className="text-zinc-500 dark:text-zinc-400">Amount (USD)</dt>
              ...
              {signal.supplyPercentage !== undefined && (
                <div className="flex items-center justify-between">
                  ...
                  <dd className="font-medium text-zinc-900 dark:text-zinc-50">
                    {signal.supplyPercentage.toFixed(4)}%
                  </dd>
                </div>
              )}
              {signal.liquidityRatio !== undefined && (
                <div className="flex items-center justify-between">
                  ...
                  <dd className="font-medium text-zinc-900 dark:text-zinc-50">
                    {signal.liquidityRatio.toFixed(2)}x
                  </dd>
                </div>
              )}
            </dl>
